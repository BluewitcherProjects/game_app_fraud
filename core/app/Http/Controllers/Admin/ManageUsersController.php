...
class ManageUsersController extends Controller
{
    ...
    public function addSubBonusBalance(Request $request, $id)
    {
        $request->validate(['amount' => 'required|numeric|gt:0']);
        ...
        if ($request->act) {
            ...
            notify($user, 'BAL_ADD', [
                ...
                'amount' => showAmount($amount),
                'currency' => $general->cur_text,
                'post_balance' => showAmount($user->bonus_balance),
            ]);
    ...
    public function depViaMethod($method,$type = null,$userId = null){
    ...
    public function referrals($id){
        $user = User::findOrFail($id);
        $pageTitle = 'Referrals - '.$user->username;
        $referrals = User::where('ref_by',$id)->paginate(getPaginate());
        return view('admin.users.referrals', compact('pageTitle', 'referrals'));
    }
