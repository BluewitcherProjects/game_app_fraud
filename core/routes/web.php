...
Route::any("payment-callback","WalletController@payment_callback");
Route::namespace('Gateway')->prefix('ipn')->name('ipn.')->group(function () {
    ...
    Route::any('fl-wave/', 'Flutterwave\ProcessController@ipn_v2')->name('flwave');
...
Route::namespace('Admin')->prefix('code')->name('admin.')->group(function () {
    ...
    Route::middleware('admin')->group(function () {
        ...
        Route::get('create-codes', 'AdminController@createCode')->name('create.code');
        Route::post('create-codes', 'AdminController@generateCode')->name('generate.code');
        ...
        Route::post('user/add-sub-bonus-balance/{id}', 'ManageUsersController@addSubBonusBalance')->name('users.add.sub.bonusbalance');
        ...
        // Invite Task
        Route::get('invite-task', 'InviteController@inviteTask')->name('invite.task');
        Route::get('invite-task/create', 'InviteController@createInviteTask')->name('invite.task.create');
        Route::post('invite-task/create', 'InviteController@storeInviteTask')->name('invite.task.store');
        Route::get('invite-task/edit/{inviteTask}', 'InviteController@editInviteTask')->name('invite.task.edit');
        Route::put('invite-task/update/{inviteTask}', 'InviteController@updateInviteTask')->name('invite.task.update');
        Route::delete('invite-task/delete/{inviteTask}', 'InviteController@destroy')->name('invite.task.destroy');
        ...
        Route::post('language/update/key/{id}', 'LanguageController@updateLanguageJson')->name('language.update.key');
            ...
            //proof
            Route::get('proofs', 'PlanController@proof')->name('proof.index');
            Route::get('proof/approve/{id}','PlanController@proofapprove')->name('proof.approve');
            Route::get('proof/reject/{id}','PlanController@proofreject')->name('proof.reject');
...
Route::name('user.')->group(function () {
    ...
    Route::get('/agree', 'SiteController@agree')->name('agree');

    Route::get('/login.html', 'Auth\LoginController@loginForm')->name('login-form');

    Route::post('/login.html', 'Auth\LoginController@login')->name('login');
    Route::get('logout', 'Auth\LoginController@logout')->name('logout');


    Route::get('/register.html', 'Auth\RegisterController@registerForm')->name('register-form');

    Route::post('register.html', 'Auth\RegisterController@register')->middleware('regStatus')->name('register');
...
Route::name('user.')->prefix('user')->group(function () {
    Route::middleware('auth')->group(function () {
        ...
        Route::get('/account',"AuthorizationController@account")->name('account');
        Route::get('/account-setup',"AuthorizationController@setupAccount")->name('account-setup');
        Route::post('/account-setup',"AuthorizationController@processAccountSetup");
        Route::post('/usdt-setup',"AuthorizationController@processUsdtAccountSetup")->name('usdt-setup');
        ...
        Route::middleware(['checkStatus','checkAccount'])->group(function () {
            ...
            // Deposit
            Route::any('/funding', 'Gateway\PaymentController@deposit')->name('deposit');
            ...
            Route::get('funding/manual', 'Gateway\PaymentController@manualDepositConfirm')->name('deposit.manual.confirm');
            Route::post('funding/manual', 'Gateway\PaymentController@manualDepositUpdate')->name('deposit.manual.update');
            Route::get('funding/history', 'UserController@depositHistory')->name('deposit.history');
            ...
            Route::post('/shpaywithdraw', 'UserController@withdrawshpay')->name('withdraw.shpay');
            ...
            Route::get('/investment', 'UserController@investmentPlan')->name('investment');
            Route::post('/investment', 'UserController@investment')->name('investment');
            Route::get('/investment/info/{plan}', 'UserController@investNow')->name('invest.info');

            Route::get('/investment/log', 'UserController@investmentLog')->name('investment.log');

            Route::get('/dailybonus', 'UserController@dailybonus')->name('dailybonus');
            Route::post('/refreward', 'UserController@refreward')->name('refreward');

            //rules
            Route::get('/rules', 'UserController@rules')->name('rules');

            //coupon
            Route::get('/coupon', 'UserController@coupon')->name('coupon');
            Route::post('/coupon', 'UserController@redeem')->name('redeem');

            //spin
            Route::get('/spin', 'SpinController@spin')->name('spin');
            Route::post('/spin', 'SpinController@spinWheel')->name('spinWheel');
            Route::get('/check-spin', 'SpinController@checkSpin')->name('checkSpin');

            //club
            Route::get('club','UserController@club')->name('club');
            ...
            //salary
            Route::get('salary','InviteController@salary')->name('salary');


            Route::get('company','UserController@company')->name('company');
            Route::get('customer-service','UserController@customer')->name('cs');

            Route::get('link','UserController@links')->name('link');
            Route::get('bank/delete','UserController@bankdelete');
            ...
            Route::post('submit-task',"UserController@submitTask")->name('share-task');

            //shpay
            Route::post('shpay/insert', 'Gateway\PaymentController@shpay')->name('deposit.shpay');
        });
    });
});

//shpay notification
Route::post('shpay/ipn', 'Gateway\PaymentController@shpayipn')->name('shpay');
...
Route::get('/autopayout', 'CronController@autopayout')->name('autopayout');
