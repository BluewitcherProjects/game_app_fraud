...
<div class="row justify-content-center">
    <div class="col-xl-4 col-sm-6 mb-30">
        <div class="widget-two box--shadow2 b-radius--5 bg--6">
            <div class="widget-two__content">
                <h2 class="text-white">{{ __($general->cur_sym) }}{{ $total_pending }}</h2>
    ...
    <div class="col-xl-4 col-sm-6 mb-30">
        <div class="widget-two box--shadow2 b-radius--5 bg--6">
            <div class="widget-two__content">
                <h2 class="text-white">{{ __($general->cur_sym) }}{{ $balance_amount}}</h2>
                <p class="text-white">@lang('Flutterwave Balance')</p>
            </div>
        </div><!-- widget-two end -->
    </div>
    <div class="col-lg-12">
                            ...
                            <tr>
                                    ...
                                    @if($details != null)
                                        @foreach(json_decode($details) as $k => $val)
                                            ...
                                            <div>
                                                ...
                                                <span>{{inputTitle($k)}}</span>=><span>{{$val->field_name}}</span>

                                                @if($k == "bank_name")
                                                    ({{ get_bank_name($val->field_name) }})
                                                @endif
                                            </div>
                                ...
                                <td data-label="@lang('Action')">
                                    ...
                                    @if($withdraw->status == 2)
                                        ...
                                        <form action="{{ route('admin.withdraw.approve') }}" method="POST"  style="display: inline-block">
                                            @csrf
                                            <input type="hidden" name="id" value="{{ $withdraw->id }}">
                                            <input type="hidden" name="details" value="Paid" />
                                            <button type="submit" class="btn btn--success btn-sm">@lang('Pay Auto')</button>
                                        </form>
                                        ...
                                        <form action="{{ route('admin.withdraw.approve') }}" method="POST"  style="display: inline-block">
                                            ...
                                            <button type="submit" class="btn btn--success btn-sm">@lang('Approve Manually')</button>
                                        </form>
